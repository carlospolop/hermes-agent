...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # not a provider failure. Classify the detailed legacy form before the
    # generic timeout branch so operators are not told that the fallback chain
    # was exhausted.
    ...
    # Newer inactivity messages use the deterministic "idle for Ns (limit Ns)"
    # shape and do not necessarily include a last-activity description.
    ...
    # TERMINAL_CWD lock waits are scheduler-internal, not provider timeouts.
    ...
    # A no_agent job never reaches a provider, so only provider-backed jobs may
    # use the provider-timeout classification.
    if provider_reachable and (
